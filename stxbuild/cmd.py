...
def main():
    ...
    parser.add_argument("--repodir", help="input the repo dir", default=os.getcwd())
    ...
    if not os.path.exists(args.repodir):
        os.makedirs(args.repodir)
    repo = args.repo.rstrip('/')
    repo_name = os.path.splitext(os.path.basename(repo))[0]
    repo_path = os.path.join(args.repodir, repo_name)
    ...
    git_repo = RepoManager(repo, path=repo_path)
    git_repo.checkout(branch=args.branch)
    build_mgr = BuildManager(work_path, repo_path, args.type)
